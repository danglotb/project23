...
class Window {
	...
	static void initialize(html.CanvasElement canvas, utils.Vector2D margin) {
		...
		canvas.addEventListener('mousemove', (e) => _instance._dispatchEvent(new Event(EventType.MOUSE_MOVE, mousePosition: new utils.Coordinates2D(e.client.x - _instance._offset.x, e.client.y - _instance._offset.y))));
		canvas.addEventListener('mousedown', (e) => _instance._dispatchEvent(new Event(EventType.MOUSE_PUSH, mousePosition: new utils.Coordinates2D(e.client.x - _instance._offset.x, e.client.y - _instance._offset.y))));
		canvas.addEventListener('mouseup', (e) => _instance._dispatchEvent(new Event(EventType.MOUSE_RELEASE, mousePosition: new utils.Coordinates2D(e.client.x - _instance._offset.x, e.client.y - _instance._offset.y))));
		canvas.addEventListener('mouseout', (e) => _instance._dispatchEvent(new Event(EventType.MOUSE_OUT)));
		html.window.addEventListener('keypress', (e) {
			if (e.charCode != 0) _instance._dispatchEvent(new Event(EventType.KEY_PRESSED, charCode: e.charCode));
		});
	...
	void setContent(ui.Component component) {
		...
		_content = component;
		_content.setPositionMode(ui.PositionMode.ABSOLUTE);
